...
Disa_Aktar_Menu(playerid)
{
    ...
    Dialog_Show(playerid, DIALOG_EXPORT, DIALOG_STYLE_LIST, "Export Type", "SA-MP (default)\nOPEN.MP\nKAINURE {10eb2e}(NEW!)", "Select", "Cancel");
    return 1;
}

new type_export; // 0 - samp, 1 - open, 2 - kainure

Dialog:DIALOG_EXPORT(playerid, response, listitem, inputtext[])
{
    if(!response) return Dialog_Show(playerid, DIALOG_EXPORT, DIALOG_STYLE_LIST, "Export Type", "SA-MP (default)\nOPEN.MP\nKAINURE {10eb2e}(NEW!)", "Select", "Cancel");
    else
    {
        type_export = listitem;
...
Dialog:DOSYA_DAKTAR(playerid, response, listitem, inputtext[])
{
    ...
    if(response)
    {
        if(isnull(inputtext))
        {
            if(type_export == 1)  Export(playerid, proje_adi, 1, 0); // open.mp
            else if(type_export == 2)  Export(playerid, proje_adi, 0, 1); // kainure
            else Export(playerid, proje_adi); // sa-mp            
        }
        else
        {
            ...
            if(type_export == 1)  Export(playerid, inputtext, 1, 0); // open.mp
            else if(type_export == 2)  Export(playerid, inputtext, 0, 1); // kainure
            else Export(playerid, inputtext); // sa-mp
        }
    }
    return 1;
}

stock Export(playerid, const dosya_adi[], open_mp_support = 0, kainure_support = 0)
{
    new
        File: dosya,
        index,
    ...
    else if(kainure_support == 1)
    {
        format(fName, sizeof(fName), "%s-kainure", dosya_adi);
        dosya = fopen(fex(DOSYA_DISA_AKTAR, fName), io_write);
    }
    else
    ...
    if(g)
    {
        ...
        if(kainure_support) format(sz, sizeof(sz), "let %s: number[] = new Array(%d);\r\n\r\n", Text_Global, Total_gPlayer2(0));
        else format(sz, sizeof(sz), "new Text: %s[%d];\r\n\r\n", Text_Global, Total_gPlayer2(0));
        fwrite(dosya, sz);
        ...
        foreach(new id : Text_List)
        {
            ...
            {
                if(kainure_support) format(sz, sizeof(sz), "%s[%d] = Native.TextDrawCreate(%.3f, %.3f, \"%s\");\r\n", Text_Global, index, Textler[id][text.pos][0], Textler[id][text.pos][1], Textler[id][text.string]);
                else format(sz, sizeof(sz), "%s[%d] = TextDrawCreate(%.3f, %.3f, \"%s\");\r\n", Text_Global, index, Textler[id][text.pos][0], Textler[id][text.pos][1], Textler[id][text.string]);
                fwrite(dosya, sz);
                
                if(Textler[id][text.lettersize][0] != 0.0 || Textler[id][text.lettersize][1] != 0.0)
                {
                    if(kainure_support) format(sz, sizeof(sz), "Native.TextDrawLetterSize(%s[%d], %.3f, %.3f);\r\n", Text_Global, index, Textler[id][text.lettersize][0], Textler[id][text.lettersize][1]);
                    else format(sz, sizeof(sz), "TextDrawLetterSize(%s[%d], %.3f, %.3f);\r\n", Text_Global, index, Textler[id][text.lettersize][0], Textler[id][text.lettersize][1]);
                    fwrite(dosya, sz);
                }

                if(Textler[id][text.textsize][0] != 0.0 || Textler[id][text.textsize][1] != 0.0)
                {
                    if(kainure_support) format(sz, sizeof(sz), "Native.TextDrawTextSize(%s[%d], %.3f, %.3f);\r\n", Text_Global, index, Textler[id][text.textsize][0], Textler[id][text.textsize][1]);
                    else format(sz, sizeof(sz), "TextDrawTextSize(%s[%d], %.3f, %.3f);\r\n", Text_Global, index, Textler[id][text.textsize][0], Textler[id][text.textsize][1]);
                    fwrite(dosya, sz);
                }

                if(kainure_support == 1) format(sz, sizeof(sz), "Native.TextDrawAlignment(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.alignment]);
                else if(open_mp_support == 1) format(sz, sizeof(sz), "TextDrawAlignment(%s[%d], %s);\r\n", Text_Global, index, OpenMP_Alignment(id));
                else format(sz, sizeof(sz), "TextDrawAlignment(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.alignment]);
                fwrite(dosya, sz);

                if(open_mp_support == 1) format(sz, sizeof(sz), "TextDrawColour(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.color]);
                else if(kainure_support == 1) format(sz, sizeof(sz), "Native.TextDrawColor(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.color]);
                else format(sz, sizeof(sz), "TextDrawColor(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.color]);
                fwrite(dosya, sz);

                if(Textler[id][text.usebox] == 1)
                {
                    if(kainure_support == 1) format(sz, sizeof(sz), "Native.TextDrawUseBox(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.usebox]);
                    else if(open_mp_support == 1) format(sz, sizeof(sz), "TextDrawUseBox(%s[%d], %s);\r\n", Text_Global, index, (Textler[id][text.usebox]) ? ("true") : ("false"));
                    else format(sz, sizeof(sz), "TextDrawUseBox(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.usebox]);
                    fwrite(dosya, sz);

                    if(open_mp_support == 1) format(sz, sizeof(sz), "TextDrawBoxColour(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.boxcolor]);
                    else if(kainure_support == 1) format(sz, sizeof(sz), "Native.TextDrawBoxColor(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.boxcolor]);
                    else format(sz, sizeof(sz), "TextDrawBoxColor(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.boxcolor]);
                    fwrite(dosya, sz);
                }

                if(kainure_support) format(sz, sizeof(sz), "Native.TextDrawSetShadow(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.shadow]);
                else format(sz, sizeof(sz), "TextDrawSetShadow(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.shadow]);
                fwrite(dosya, sz);

                if(kainure_support) format(sz, sizeof(sz), "Native.TextDrawSetOutline(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.outline]);
                else format(sz, sizeof(sz), "TextDrawSetOutline(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.outline]);
                fwrite(dosya, sz);

                if(open_mp_support == 1) format(sz, sizeof(sz), "TextDrawBackgroundColour(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.bgcolor]);
                else if(kainure_support == 1) format(sz, sizeof(sz), "Native.TextDrawBackgroundColor(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.bgcolor]);
                else format(sz, sizeof(sz), "TextDrawBackgroundColor(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.bgcolor]);
                fwrite(dosya, sz);

                if(open_mp_support == 1) format(sz, sizeof(sz), "TextDrawFont(%s[%d], %s);\r\n", Text_Global, index, OpenMP_Font(id));
                if(kainure_support == 1) format(sz, sizeof(sz), "Native.TextDrawFont(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.font]);
                else format(sz, sizeof(sz), "TextDrawFont(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.font]);
                fwrite(dosya, sz);
                ...
                if(kainure_support == 1) format(sz, sizeof(sz), "Native.TextDrawSetProportional(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.proportional]);
                else format(sz, sizeof(sz), "TextDrawSetProportional(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.proportional]);
                fwrite(dosya, sz);

                if(Textler[id][text.font] == TEXT_DRAW_FONT_MODEL_PREVIEW)
                {
                    if(kainure_support) format(sz, sizeof(sz), "Native.TextDrawSetPreviewModel(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.preview_model]);
                    else format(sz, sizeof(sz), "TextDrawSetPreviewModel(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.preview_model]);
                    fwrite(dosya, sz);

                    if(kainure_support) format(sz, sizeof(sz), "Native.TextDrawSetPreviewRot(%s[%d], %.3f, %.3f, %.3f, %.3f);\r\n", Text_Global, index, Textler[id][text.preview_rot][0], Textler[id][text.preview_rot][1], Textler[id][text.preview_rot][2], Textler[id][text.preview_rot][3]);
                    else format(sz, sizeof(sz), "TextDrawSetPreviewRot(%s[%d], %.3f, %.3f, %.3f, %.3f);\r\n", Text_Global, index, Textler[id][text.preview_rot][0], Textler[id][text.preview_rot][1], Textler[id][text.preview_rot][2], Textler[id][text.preview_rot][3]);
                    fwrite(dosya, sz);
                    ...
                    else if(kainure_support == 1) format(sz, sizeof(sz), "Native.TextDrawSetPreviewVehCol(%s[%d], %d, %d);\r\n", Text_Global, index, Textler[id][text.preview_vc][0], Textler[id][text.preview_vc][1]);
                    else format(sz, sizeof(sz), "TextDrawSetPreviewVehCol(%s[%d], %d, %d);\r\n", Text_Global, index, Textler[id][text.preview_vc][0], Textler[id][text.preview_vc][1]);
                    fwrite(dosya, sz);
                }

                if(Textler[id][text.selectable] == 1)
                {
                    if(kainure_support == 1) format(sz, sizeof(sz), "Native.TextDrawSetSelectable(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.selectable]);
                    else if(open_mp_support == 1) format(sz, sizeof(sz), "TextDrawSetSelectable(%s[%d], %s);\r\n", Text_Global, index, (Textler[id][text.selectable]) ? ("true") : ("false"));
                    else format(sz, sizeof(sz), "TextDrawSetSelectable(%s[%d], %d);\r\n", Text_Global, index, Textler[id][text.selectable]);
                    fwrite(dosya, sz);
                }
    ...
    if(p)
    {
        ...
        if(kainure_support) format(sz, sizeof(sz), "let %s: number[][] = Array.from({ length: MAX_PLAYERS }, () => new Array<number>(%d).fill(-1));\r\n\r\n", Text_Player, Total_gPlayer2(1));
        else format(sz, sizeof(sz), "new PlayerText: %s[MAX_PLAYERS][%d];\r\n\r\n", Text_Player, Total_gPlayer2(1));
        fwrite(dosya, sz);
        foreach(new id : Text_List)
        {
            if(Textler[id][text.globaplayer] == 1 && !strlen(Textler[id][text.degiskenAdi]))
            {
                if(kainure_support) format(sz, sizeof(sz), "%s[playerid][%d] = Native.CreatePlayerTextDraw(playerid, %.3f, %.3f, \"%s\");\r\n", Text_Player, index, Textler[id][text.pos][0], Textler[id][text.pos][1], Textler[id][text.string]);
                else format(sz, sizeof(sz), "%s[playerid][%d] = CreatePlayerTextDraw(playerid, %.3f, %.3f, \"%s\");\r\n", Text_Player, index, Textler[id][text.pos][0], Textler[id][text.pos][1], Textler[id][text.string]);
                fwrite(dosya, sz);
                
                if(Textler[id][text.lettersize][0] != 0.0 || Textler[id][text.lettersize][1] != 0.0)
                {
                    if(kainure_support) format(sz, sizeof(sz), "Native.PlayerTextDrawLetterSize(playerid, %s[playerid][%d], %.3f, %.3f);\r\n", Text_Player, index, Textler[id][text.lettersize][0], Textler[id][text.lettersize][1]);
                    else format(sz, sizeof(sz), "PlayerTextDrawLetterSize(playerid, %s[playerid][%d], %.3f, %.3f);\r\n", Text_Player, index, Textler[id][text.lettersize][0], Textler[id][text.lettersize][1]);
                    fwrite(dosya, sz);
                }

                if(Textler[id][text.textsize][0] != 0.0 || Textler[id][text.textsize][1] != 0.0)
                {
                    if(kainure_support) format(sz, sizeof(sz), "Native.PlayerTextDrawTextSize(playerid, %s[playerid][%d], %.3f, %.3f);\r\n", Text_Player, index, Textler[id][text.textsize][0], Textler[id][text.textsize][1]);
                    else format(sz, sizeof(sz), "PlayerTextDrawTextSize(playerid, %s[playerid][%d], %.3f, %.3f);\r\n", Text_Player, index, Textler[id][text.textsize][0], Textler[id][text.textsize][1]);
                    fwrite(dosya, sz);
                }
                ...
                else if(kainure_support == 1) format(sz, sizeof(sz), "Native.PlayerTextDrawAlignment(playerid, %s[playerid][%d], %d);\r\n", Text_Player, index, Textler[id][text.alignment]);
                else format(sz, sizeof(sz), "PlayerTextDrawAlignment(playerid, %s[playerid][%d], %d);\r\n", Text_Player, index, Textler[id][text.alignment]);
                fwrite(dosya, sz);
                ...
                else if(kainure_support == 1) format(sz, sizeof(sz), "Native.PlayerTextDrawColor(playerid, %s[playerid][%d], %d);\r\n", Text_Player, index, Textler[id][text.color]);
                else format(sz, sizeof(sz), "PlayerTextDrawColor(playerid, %s[playerid][%d], %d);\r\n", Text_Player, index, Textler[id][text.color]);
                fwrite(dosya, sz);
                ...
                if(Textler[id][text.usebox] == 1)
                {
                    ...
                    else if(kainure_support == 1) format(sz, sizeof(sz), "Native.PlayerTextDrawUseBox(playerid, %s[playerid][%d], %d);\r\n", Text_Player, index, Textler[id][text.usebox]);
                    else format(sz, sizeof(sz), "PlayerTextDrawUseBox(playerid, %s[playerid][%d], %d);\r\n", Text_Player, index, Textler[id][text.usebox]);
                    fwrite(dosya, sz);
                    ...
                    else if(kainure_support == 1) format(sz, sizeof(sz), "Native.PlayerTextDrawBoxColor(playerid, %s[playerid][%d], %d);\r\n", Text_Player, index, Textler[id][text.boxcolor]);
                    else format(sz, sizeof(sz), "PlayerTextDrawBoxColor(playerid, %s[playerid][%d], %d);\r\n", Text_Player, index, Textler[id][text.boxcolor]);
                    fwrite(dosya, sz);
                }

                if(kainure_support) format(sz, sizeof(sz), "Native.PlayerTextDrawSetShadow(playerid, %s[playerid][%d], %d);\r\n", Text_Player, index, Textler[id][text.shadow]);
                else format(sz, sizeof(sz), "PlayerTextDrawSetShadow(playerid, %s[playerid][%d], %d);\r\n", Text_Player, index, Textler[id][text.shadow]);
                fwrite(dosya, sz);

                if(kainure_support) format(sz, sizeof(sz), "Native.PlayerTextDrawSetOutline(playerid, %s[playerid][%d], %d);\r\n", Text_Player, index, Textler[id][text.outline]);
                else format(sz, sizeof(sz), "PlayerTextDrawSetOutline(playerid, %s[playerid][%d], %d);\r\n", Text_Player, index, Textler[id][text.outline]);
                fwrite(dosya, sz);
                ...
                else if(kainure_support == 1) format(sz, sizeof(sz), "Native.PlayerTextDrawBackgroundColor(playerid, %s[playerid][%d], %d);\r\n", Text_Player, index, Textler[id][text.bgcolor]);
                else format(sz, sizeof(sz), "PlayerTextDrawBackgroundColor(playerid, %s[playerid][%d], %d);\r\n", Text_Player, index, Textler[id][text.bgcolor]);
                fwrite(dosya, sz);

                if(open_mp_support == 1) format(sz, sizeof(sz), "PlayerTextDrawFont(playerid, %s[playerid][%d], %s);\r\n", Text_Player, index, OpenMP_Font(id));
                else if(kainure_support == 1) format(sz, sizeof(sz), "Native.PlayerTextDrawFont(playerid, %s[playerid][%d], %d);\r\n", Text_Player, index, Textler[id][text.font]);
                else format(sz, sizeof(sz), "PlayerTextDrawFont(playerid, %s[playerid][%d], %d);\r\n", Text_Player, index, Textler[id][text.font]);
                fwrite(dosya, sz);
                ...
                else if(kainure_support == 1) format(sz, sizeof(sz), "Native.PlayerTextDrawSetProportional(playerid, %s[playerid][%d], %d);\r\n", Text_Player, index, Textler[id][text.proportional]);
                else format(sz, sizeof(sz), "PlayerTextDrawSetProportional(playerid, %s[playerid][%d], %d);\r\n", Text_Player, index, Textler[id][text.proportional]);
                fwrite(dosya, sz);

                if(Textler[id][text.font] == TEXT_DRAW_FONT_MODEL_PREVIEW)
                {
                    if(kainure_support == 1)
                    {
                        format(sz, sizeof(sz), "Native.PlayerTextDrawSetPreviewModel(playerid, %s[playerid][%d], %d);\r\n", Text_Player, index, Textler[id][text.preview_model]);
                        fwrite(dosya, sz);

                        format(sz, sizeof(sz), "Native.PlayerTextDrawSetPreviewRot(playerid, %s[playerid][%d], %.3f, %.3f, %.3f, %.3f);\r\n", Text_Player, index, Textler[id][text.preview_rot][0], Textler[id][text.preview_rot][1], Textler[id][text.preview_rot][2], Textler[id][text.preview_rot][3]);
                        fwrite(dosya, sz);
                    }   
                    else
                    {
                    ...
                    else if(kainure_support == 1) format(sz, sizeof(sz), "Native.PlayerTextDrawSetPreviewVehCol(playerid, %s[playerid][%d], %d, %d);\r\n", Text_Player, index, Textler[id][text.preview_vc][0], Textler[id][text.preview_vc][1]);
                    else format(sz, sizeof(sz), "PlayerTextDrawSetPreviewVehCol(playerid, %s[playerid][%d], %d, %d);\r\n", Text_Player, index, Textler[id][text.preview_vc][0], Textler[id][text.preview_vc][1]);
                    fwrite(dosya, sz);
                }
                ...
                if(Textler[id][text.selectable] == 1)
                {
                    ...
                    else if(kainure_support == 1) format(sz, sizeof(sz), "Native.PlayerTextDrawSetSelectable(playerid, %s[playerid][%d], %d);\r\n", Text_Player, index, Textler[id][text.selectable]);
                    else format(sz, sizeof(sz), "PlayerTextDrawSetSelectable(playerid, %s[playerid][%d], %d);\r\n", Text_Player, index, Textler[id][text.selectable]);
                    fwrite(dosya, sz);
                }
    ...
    if(dg)
    {
        ...
        foreach(new id : Text_List)
        {
            if(Textler[id][text.globaplayer] == 0 && strlen(Textler[id][text.degiskenAdi]))
            {
                if(kainure_support) format(sz, sizeof(sz), "let %s: number;\r\n", Textler[id][text.degiskenAdi]);
                else format(sz, sizeof(sz), "new Text: %s;\r\n", Textler[id][text.degiskenAdi]);
                fwrite(dosya, sz);
            }
        }
        fwrite(dosya, "\r\n");
        foreach(new id : Text_List)
        {
            if(Textler[id][text.globaplayer] == 0 && strlen(Textler[id][text.degiskenAdi]))
            {
                if(kainure_support) format(sz, sizeof(sz), "%s = Native.TextDrawCreate(%.3f, %.3f, \"%s\");\r\n", Textler[id][text.degiskenAdi], Textler[id][text.pos][0], Textler[id][text.pos][1], Textler[id][text.string]);
                else format(sz, sizeof(sz), "%s = TextDrawCreate(%.3f, %.3f, \"%s\");\r\n", Textler[id][text.degiskenAdi], Textler[id][text.pos][0], Textler[id][text.pos][1], Textler[id][text.string]);
                fwrite(dosya, sz);
                
                if(Textler[id][text.lettersize][0] != 0.0 || Textler[id][text.lettersize][1] != 0.0)
                {
                    if(kainure_support) format(sz, sizeof(sz), "Native.TextDrawLetterSize(%s, %.3f, %.3f);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.lettersize][0], Textler[id][text.lettersize][1]);
                    else format(sz, sizeof(sz), "TextDrawLetterSize(%s, %.3f, %.3f);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.lettersize][0], Textler[id][text.lettersize][1]);
                    fwrite(dosya, sz);
                }

                if(Textler[id][text.textsize][0] != 0.0 || Textler[id][text.textsize][1] != 0.0)
                {
                    if(kainure_support) format(sz, sizeof(sz), "Native.TextDrawTextSize(%s, %.3f, %.3f);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.textsize][0], Textler[id][text.textsize][1]);
                    else format(sz, sizeof(sz), "TextDrawTextSize(%s, %.3f, %.3f);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.textsize][0], Textler[id][text.textsize][1]);
                    fwrite(dosya, sz);
                }

                if(open_mp_support == 1) format(sz, sizeof(sz), "TextDrawAlignment(%s, %s);\r\n", Textler[id][text.degiskenAdi], OpenMP_Alignment(id));
                else if(kainure_support == 1) format(sz, sizeof(sz), "Native.TextDrawAlignment(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.alignment]);
                else format(sz, sizeof(sz), "TextDrawAlignment(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.alignment]);
                fwrite(dosya, sz);

                if(open_mp_support == 1) format(sz, sizeof(sz), "TextDrawColour(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.color]);
                else if(kainure_support == 1) format(sz, sizeof(sz), "Native.TextDrawColor(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.color]);
                else format(sz, sizeof(sz), "TextDrawColor(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.color]);
                fwrite(dosya, sz);

                if(Textler[id][text.usebox] == 1)
                {
                    if(open_mp_support == 1) format(sz, sizeof(sz), "TextDrawUseBox(%s, %s);\r\n", Textler[id][text.degiskenAdi], (Textler[id][text.usebox]) ? ("true") : ("false"));
                    else if(kainure_support == 1) format(sz, sizeof(sz), "Native.TextDrawUseBox(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.usebox]);
                    else format(sz, sizeof(sz), "TextDrawUseBox(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.usebox]);
                    fwrite(dosya, sz);

                    if(open_mp_support == 1) format(sz, sizeof(sz), "TextDrawBoxColour(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.boxcolor]);
                    else if(kainure_support == 1) format(sz, sizeof(sz), "Native.TextDrawBoxColor(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.boxcolor]);
                    else format(sz, sizeof(sz), "TextDrawBoxColor(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.boxcolor]);
                    fwrite(dosya, sz);
                }

                if(kainure_support == 1) format(sz, sizeof(sz), "Native.TextDrawSetShadow(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.shadow]);
                else format(sz, sizeof(sz), "TextDrawSetShadow(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.shadow]);
                fwrite(dosya, sz);

                if(kainure_support == 1) format(sz, sizeof(sz), "Native.TextDrawSetOutline(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.outline]);
                else format(sz, sizeof(sz), "TextDrawSetOutline(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.outline]);
                fwrite(dosya, sz);

                if(open_mp_support == 1) format(sz, sizeof(sz), "TextDrawBackgroundColour(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.bgcolor]);
                else if(kainure_support == 1) format(sz, sizeof(sz), "Native.TextDrawBackgroundColor(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.bgcolor]);
                else format(sz, sizeof(sz), "TextDrawBackgroundColor(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.bgcolor]);
                fwrite(dosya, sz);

                if(open_mp_support == 1) format(sz, sizeof(sz), "TextDrawFont(%s, %s);\r\n", Textler[id][text.degiskenAdi], OpenMP_Font(id));
                else if(kainure_support == 1) format(sz, sizeof(sz), "Native.TextDrawFont(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.font]);
                else format(sz, sizeof(sz), "TextDrawFont(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.font]);
                fwrite(dosya, sz);

                if(kainure_support == 1) format(sz, sizeof(sz), "Native.TextDrawSetProportional(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.proportional]);
                else if(open_mp_support == 1) format(sz, sizeof(sz), "TextDrawSetProportional(%s, %s);\r\n", Textler[id][text.degiskenAdi], (Textler[id][text.proportional]) ? ("true") : ("false"));
                else format(sz, sizeof(sz), "TextDrawSetProportional(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.proportional]);
                fwrite(dosya, sz);

                if(Textler[id][text.font] == TEXT_DRAW_FONT_MODEL_PREVIEW)
                {
                    if(kainure_support) format(sz, sizeof(sz), "Native.TextDrawSetPreviewModel(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.preview_model]);
                    else format(sz, sizeof(sz), "TextDrawSetPreviewModel(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.preview_model]);
                    fwrite(dosya, sz);

                    if(kainure_support) format(sz, sizeof(sz), "Native.TextDrawSetPreviewRot(%s, %.3f, %.3f, %.3f, %.3f);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.preview_rot][0], Textler[id][text.preview_rot][1], Textler[id][text.preview_rot][2], Textler[id][text.preview_rot][3]);
                    else format(sz, sizeof(sz), "TextDrawSetPreviewRot(%s, %.3f, %.3f, %.3f, %.3f);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.preview_rot][0], Textler[id][text.preview_rot][1], Textler[id][text.preview_rot][2], Textler[id][text.preview_rot][3]);
                    fwrite(dosya, sz);
                    ...
                    else if(kainure_support == 1) format(sz, sizeof(sz), "Native.TextDrawSetPreviewVehCol(%s, %d, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.preview_vc][0], Textler[id][text.preview_vc][1]);
                    else format(sz, sizeof(sz), "TextDrawSetPreviewVehCol(%s, %d, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.preview_vc][0], Textler[id][text.preview_vc][1]);
                    fwrite(dosya, sz);
                }
                ...
                if(Textler[id][text.selectable] == 1)
                {
                    ...
                    else if(kainure_support == 1) format(sz, sizeof(sz), "Native.TextDrawSetSelectable(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.selectable]);
                    else format(sz, sizeof(sz), "TextDrawSetSelectable(%s, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.selectable]);
                    fwrite(dosya, sz);
                }
    ...
    if(dp)
    {
        ...
        foreach(new id : Text_List)
        {
            if(Textler[id][text.globaplayer] == 1 && strlen(Textler[id][text.degiskenAdi]))
            {
                if(kainure_support) format(sz, sizeof(sz), "let %s: number[] = new Array.from({ length: MAX_PLAYERS }, () => []);\r\n", Textler[id][text.degiskenAdi]);
                else format(sz, sizeof(sz), "new PlayerText: %s[MAX_PLAYERS];\r\n", Textler[id][text.degiskenAdi]);
                fwrite(dosya, sz);
            }
        }
        fwrite(dosya, "\r\n");
        foreach(new id : Text_List)
        {
            if(Textler[id][text.globaplayer] == 1 && strlen(Textler[id][text.degiskenAdi]))
            {
                if(kainure_support) format(sz, sizeof(sz), "%s[playerid] = Native.CreatePlayerTextDraw(playerid, %.3f, %.3f, \"%s\");\r\n", Textler[id][text.degiskenAdi], Textler[id][text.pos][0], Textler[id][text.pos][1], Textler[id][text.string]);
                else format(sz, sizeof(sz), "%s[playerid] = CreatePlayerTextDraw(playerid, %.3f, %.3f, \"%s\");\r\n", Textler[id][text.degiskenAdi], Textler[id][text.pos][0], Textler[id][text.pos][1], Textler[id][text.string]);
                fwrite(dosya, sz);
                
                if(Textler[id][text.lettersize][0] != 0.0 || Textler[id][text.lettersize][1] != 0.0)
                {
                    if(kainure_support) format(sz, sizeof(sz), "Native.PlayerTextDrawLetterSize(playerid, %s[playerid], %.3f, %.3f);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.lettersize][0], Textler[id][text.lettersize][1]);
                    else format(sz, sizeof(sz), "PlayerTextDrawLetterSize(playerid, %s[playerid], %.3f, %.3f);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.lettersize][0], Textler[id][text.lettersize][1]);
                    fwrite(dosya, sz);
                }

                if(Textler[id][text.textsize][0] != 0.0 || Textler[id][text.textsize][1] != 0.0)
                {
                    if(kainure_support == 1) format(sz, sizeof(sz), "Native.PlayerTextDrawTextSize(playerid, %s[playerid], %.3f, %.3f);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.textsize][0], Textler[id][text.textsize][1]);
                    else format(sz, sizeof(sz), "PlayerTextDrawTextSize(playerid, %s[playerid], %.3f, %.3f);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.textsize][0], Textler[id][text.textsize][1]);
                    fwrite(dosya, sz);
                }

                if(open_mp_support == 1) format(sz, sizeof(sz), "PlayerTextDrawAlignment(playerid, %s[playerid], %s);\r\n", Textler[id][text.degiskenAdi], OpenMP_Alignment(id));
                else if(kainure_support == 1) format(sz, sizeof(sz), "Native.PlayerTextDrawAlignment(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.alignment]);
                else format(sz, sizeof(sz), "PlayerTextDrawAlignment(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.alignment]);
                fwrite(dosya, sz);

                if(open_mp_support == 1) format(sz, sizeof(sz), "PlayerTextDrawColour(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.color]);
                else if(kainure_support == 1) format(sz, sizeof(sz), "Native.PlayerTextDrawColor(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.color]);
                else format(sz, sizeof(sz), "PlayerTextDrawColor(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.color]);
                fwrite(dosya, sz);

                if(Textler[id][text.usebox] == 1)
                {
                    if(open_mp_support == 1) format(sz, sizeof(sz), "PlayerTextDrawUseBox(playerid, %s[playerid], %s);\r\n", Textler[id][text.degiskenAdi], (Textler[id][text.usebox]) ? ("true") : ("false"));
                    else if(kainure_support == 1) format(sz, sizeof(sz), "Native.PlayerTextDrawUseBox(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.usebox]);
                    else format(sz, sizeof(sz), "PlayerTextDrawUseBox(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.usebox]);
                    fwrite(dosya, sz);

                    if(open_mp_support == 1) format(sz, sizeof(sz), "PlayerTextDrawBoxColour(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.boxcolor]);
                    else if(kainure_support == 1) format(sz, sizeof(sz), "Native.PlayerTextDrawBoxColor(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.boxcolor]);
                    else format(sz, sizeof(sz), "PlayerTextDrawBoxColor(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.boxcolor]);
                    fwrite(dosya, sz);
                }

                if(kainure_support) format(sz, sizeof(sz), "Native.PlayerTextDrawSetShadow(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.shadow]);
                else format(sz, sizeof(sz), "PlayerTextDrawSetShadow(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.shadow]);
                fwrite(dosya, sz);

                if(kainure_support) format(sz, sizeof(sz), "Native.PlayerTextDrawSetOutline(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.outline]);
                else format(sz, sizeof(sz), "PlayerTextDrawSetOutline(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.outline]);
                fwrite(dosya, sz);

                if(open_mp_support == 1) format(sz, sizeof(sz), "PlayerTextDrawBackgroundColour(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.bgcolor]);
                else if(kainure_support == 1) format(sz, sizeof(sz), "Native.PlayerTextDrawBackgroundColor(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.bgcolor]);
                else format(sz, sizeof(sz), "PlayerTextDrawBackgroundColor(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.bgcolor]);
                fwrite(dosya, sz);

                if(open_mp_support == 1) format(sz, sizeof(sz), "PlayerTextDrawFont(playerid, %s[playerid], %s);\r\n", Textler[id][text.degiskenAdi], OpenMP_Font(id));
                else if(kainure_support == 1) format(sz, sizeof(sz), "Native.PlayerTextDrawFont(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.font]);
                else format(sz, sizeof(sz), "PlayerTextDrawFont(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.font]);
                fwrite(dosya, sz);

                if(open_mp_support == 1) format(sz, sizeof(sz), "PlayerTextDrawSetProportional(playerid, %s[playerid], %s);\r\n", Textler[id][text.degiskenAdi], (Textler[id][text.proportional]) ? ("true") : ("false"));
                else if(kainure_support == 1) format(sz, sizeof(sz), "Native.PlayerTextDrawSetProportional(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.proportional]);
                else format(sz, sizeof(sz), "PlayerTextDrawSetProportional(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.proportional]);
                fwrite(dosya, sz);

                if(Textler[id][text.font] == TEXT_DRAW_FONT_MODEL_PREVIEW)
                {
                    if(kainure_support) format(sz, sizeof(sz), "Native.PlayerTextDrawSetPreviewModel(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.preview_model]);
                    else format(sz, sizeof(sz), "PlayerTextDrawSetPreviewModel(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.preview_model]);
                    fwrite(dosya, sz);

                    if(kainure_support) format(sz, sizeof(sz), "Native.PlayerTextDrawSetPreviewRot(playerid, %s[playerid], %.3f, %.3f, %.3f, %.3f);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.preview_rot][0], Textler[id][text.preview_rot][1], Textler[id][text.preview_rot][2], Textler[id][text.preview_rot][3]);
                    else format(sz, sizeof(sz), "PlayerTextDrawSetPreviewRot(playerid, %s[playerid], %.3f, %.3f, %.3f, %.3f);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.preview_rot][0], Textler[id][text.preview_rot][1], Textler[id][text.preview_rot][2], Textler[id][text.preview_rot][3]);
                    fwrite(dosya, sz);

                    if(open_mp_support == 1) format(sz, sizeof(sz), "PlayerTextDrawSetPreviewVehicleColours(playerid, %s[playerid], %d, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.preview_vc][0], Textler[id][text.preview_vc][1]);
                    else if(kainure_support == 1) format(sz, sizeof(sz), "Native.PlayerTextDrawSetPreviewVehCol(playerid, %s[playerid], %d, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.preview_vc][0], Textler[id][text.preview_vc][1]);
                    else format(sz, sizeof(sz), "PlayerTextDrawSetPreviewVehCol(playerid, %s[playerid], %d, %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.preview_vc][0], Textler[id][text.preview_vc][1]);
                    fwrite(dosya, sz);
                }
                ...
                if(Textler[id][text.selectable] == 1)
                {
                    ...
                    else if(kainure_support == 1) format(sz, sizeof(sz), "Native.PlayerTextDrawSetSelectable(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.selectable]);
                    else format(sz, sizeof(sz), "PlayerTextDrawSetSelectable(playerid, %s[playerid], %d);\r\n", Textler[id][text.degiskenAdi], Textler[id][text.selectable]);
                    fwrite(dosya, sz);
                }
