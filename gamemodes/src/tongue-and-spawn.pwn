...
hook OnPlayerConnect(playerid)
{
    ...
    if(Server_Version() == VER_037)
    {
        ...
        // Exibir desenhos de texto se todos os modos estiverem selecionados.
        ...
        // Se ambas as configurações de dados estiverem configuradas, instale o HUD.
...
{
    // Seleção de idioma não feita
    if(dil_secimi == false) return Dil_Paketi(playerid);

    // O modo de geração não está definido.
    if(spawn_modu == -1) return Calisma_Ortami(playerid);
...
Calisma_Ortami(playerid)
{
    Dialog_Show(playerid, CALISMA_ORTAMI, DIALOG_STYLE_LIST, Dil_Mesaji[co_baslik], "%s\n%s", Dil_Mesaji[co_buton_1], "", Dil_Mesaji[co_icerik_1], Dil_Mesaji[co_icerik_2]);
    return 1;
}
...
Dialog:CALISMA_ORTAMI(playerid, response, listitem, inputtext[])
{
    ...
    {
        ChatTemizle();
        spawn_modu = listitem; // 0->EmptyScreen, 1->SpawnMode
        TogglePlayerSpectating(playerid, false);
